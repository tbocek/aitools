...
Settings are one bash-sourceable file, `~/.config/autocut/llm.conf`: server
...
A project is a **folder** ending in `.autocut`, holding everything the session
produced. It can be moved, copied or zipped whole.

```
<name>.autocut/
  autocut.json                    the project: sources, settings, context, prompts
