...
func TestEveryLLMStepSendsTheContext(t *testing.T) {
	...
	want := []struct{ file, sig string }{
		{"step2_describe.go", "text := a.ctxBlock()"},   // the frame describer
		{"step2_transcript.go", "user := a.ctxBlock()"}, // the transcript fixer
		{"step3.go", "user := a.ctxBlock()"},            // suggest AND its audit
		{"step4.go", "user := a.ctxBlock()"},            // the narration
	}
	for _, w := range want {
	...
	// step3 builds two of them; both have to carry it
	src, err := os.ReadFile("step3.go")
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(users) != len(withCtx) {
		t.Errorf("step3.go builds %d user messages but only %d carry the context",
