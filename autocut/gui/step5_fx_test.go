...
func TestTheCameraPathSamplesThePreview(t *testing.T) {
	// the opening view (long before this clip) is what the glide departs
	// from; without it the first-view rule would pin the whole timeline on
	// the T=105 rect and there would be nothing to animate
	fx := []cutFx{
		{Kind: "view", T: 0, Cx: 0.3, Cy: 0.5, Hf: 0.8},
		{Kind: "view", T: 105, Trans: 2, Cx: 0.5, Cy: 0.5, Hf: 0.5},
	}
	...
	if p == nil || p.static() {
		t.Fatal("a gliding view built no moving camera")
	}
	wantTs := []float64{0, 5, 7, 10} // clip edges, glide start, glide end
	...
	if !rectNear(p.r[0], fullFit(srcA, outA)) {
		t.Errorf("the untouched camera is %+v, want fullFit", p.r[0])
	}
}
...
func TestTheCameraChainIsAcceptedByFfmpeg(t *testing.T) {
	...
	// a view gliding to a half-height crop, then a zoom on top: the moving
	// window, breakpoints inside the clip
	run("moving", buildCam([]cutFx{
		{Kind: "view", T: 0.2, Trans: 0.6, Cx: 0.5, Cy: 0.5, Hf: 0.5},
...
func TestProduceRunsTheCutThroughTheEffects(t *testing.T) {
	fn := funcBody(t, "step5.go", `func \(a \*App\) produceSegs\(\) \[\]cutSeg \{`)
