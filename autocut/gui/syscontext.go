...
//
// The session notes are not here at all, and that is the point. Everything
// about them -- that they outrank what a job would infer, and how they change
// the reading of the spoken lines -- travels WITH them, in the request
// (ctxBlock). A session with an empty notes box then sends none of it, where
// this prompt sent every job a paragraph about a block that was not there;
// and where there are notes, the rule sits beside them rather than six
// thousand characters earlier.
//
