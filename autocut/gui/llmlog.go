...
// chatText reads a recorded page back as text: the inverse of chatHTML, as far
// as a machine needs it. The pages are the only record of what the model was
// actually told -- the run log says a call happened and quotes its first
// hundred characters -- so a question about why a step decided something has to
// be answered from them, and a question asked of the model has to send them.
//
// Parsed rather than kept alongside as text, because a second copy of every
// exchange on disk is a second copy to go stale, and because the page is
// generated here: it holds the tags this reads and no others.
//
// Two things are dropped. The images, which are data URLs and are most of the
// bytes, become a one-word note that a picture went out -- the model being
// asked cannot see them anyway. The thinking goes the same way and for a
// stronger reason: it is the model talking to itself, it dwarfs the answer, and
// what was decided is in the answer.
func chatText(page []byte) string {
	// what to keep, and how it reads once the tags are gone
	keep := []struct{ open, close, before, after string }{
		{"<h1>", "</h1>", "step: ", "\n"},
		{`<p class="meta">`, "</p>", "", "\n"},
		{`<p class="err">`, "</p>", "", "\n"},
		{"<h2>", "</h2>", "\n--- ", " ---\n"},
	...
	if i := strings.Index(s, "<h1>"); i >= 0 {
		s = s[i:] // everything above the first heading is styling
	}
	var b strings.Builder
	for {
		...
		s = s[i:]
		if strings.HasPrefix(s, "<details>") {
			j := strings.Index(s, "</details>")
			if j < 0 {
				break // a page cut off mid-thought: there is nothing after it
			}
			s = s[j+len("</details>"):]
		...
		k, ok := keep[0], false
		for _, cand := range keep {
			if strings.HasPrefix(s, cand.open) {
				k, ok = cand, true
				break
			}
		}
		...
		s = s[len(k.open):]
		body := s
		if j := strings.Index(s, k.close); j >= 0 {
			body, s = s[:j], s[j+len(k.close):]
		} else {
			s = "" // the live page's last <pre> is left open on purpose
		}
		b.WriteString(k.before + strings.TrimRight(html.UnescapeString(body), "\n") + k.after)
	}
	return b.String()
}

// recentExchanges is the session's calls as text, newest first until budget
// runs out and then oldest first in what comes back -- a model reads forward,
// and the last thing before the question should be the last thing that
// happened. The count of what did not fit is in the text: a brief that quietly
// held half the session would be answered with the same confidence as one that
// held all of it.
//
// By modification time, not by name. The names sort chronologically within a
// year and not across one, and "the newest exchanges" is exactly the thing that
// must not silently mean "the ones from January".
func (a *App) recentExchanges(budget int) string {
	if a.outDir == "" {
		return "(no exchanges recorded: this session has not run anything yet)"
	}
	ents, err := os.ReadDir(a.llmDir())
	if err != nil {
		return "(no exchanges recorded: this session has not run anything yet)"
	}
	type page struct {
	...
	for _, e := range ents {
		...
		pages = append(pages, page{e.Name(), fi.ModTime()})
	}
	sort.Slice(pages, func(i, j int) bool { return pages[i].when.After(pages[j].when) })

	var took []string
	left, dropped := budget, 0
	for _, p := range pages {
		b, err := os.ReadFile(filepath.Join(a.llmDir(), p.name))
		if err != nil {
			continue
		}
		t := "=== " + p.name + " ===\n" + chatText(b)
		if len(t) > left {
			dropped++
			continue // a later, smaller one may still fit
		}
		left -= len(t)
		took = append(took, t)
	}
	...
	slices.Reverse(took)
	out := strings.Join(took, "\n")
	if dropped > 0 {
		out = fmt.Sprintf("(%d older or larger exchange(s) left out for length)\n\n", dropped) + out
	}
	return out
}
