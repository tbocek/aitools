...
// two halves of the effects (step3_fx.go edits them, step5_fx.go renders them).
//
...
// default one rather than a zero-sized rectangle nobody can see or grab.
...
// textsOf is the text effects of a cut, earliest first, skipping the ones that
// would show nothing: no words, or no time to show them in.
func textsOf(fx []cutFx) []cutFx {
	var out []cutFx
	for _, f := range fx {
		if f.Kind == "text" && strings.TrimSpace(f.Text) != "" && f.Dur > 0 {
			out = append(out, f)
		}
	}
	return out
...
func textCues(fx []cutFx, sessS, span, rate, length float64) []textCue {
	...
	for i, f := range fx {
		...
		if span <= 0 {
			...
			s = (f.T - sessS) / rate
			e = (f.T + f.Dur - sessS) / rate
			if e <= 0 || s >= length {
				continue
			}
			// a text that starts before this clip is already fully up by the
			// time the clip begins, so the fade it kept is the part still owing
			if s < 0 {
				fin = math.Max(0, fin/rate+s)
				s = 0
			} else {
				fin /= rate
			}
			if e > length {
				fout = math.Max(0, fout/rate-(e-length))
				e = length
			} else {
				fout /= rate
			}
...
// arrives on, base the input index of the first text file, and the label the
// picture leaves on comes back.
//
...
func textChain(cues []textCue, in string, base int) (string, string) {
	var b strings.Builder
	cur := in
	for k, c := range cues {
		...
		b.WriteString(fmt.Sprintf("[%s][tx%d]overlay=x=0:y=0:eof_action=pass:"+
			"enable=between(t\\,%.3f\\,%.3f)[%s];", cur, k, c.s, c.e, out))
		cur = out
	}
	return b.String(), cur
}
