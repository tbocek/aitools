...
func (ed *cutEditor) cardSound(s *cutSeg, into float64) {
	ed.player.SetMuted(s != nil)
	want := ed.cardVoice(s)
	if want == ed.cardSnd {
		return
	}
	ed.cardSnd = want
	ed.player.CardSound(want, into, want != "")
}
...
// an inserted video's audio, and nothing at all for a still or a drawn card,
// which have none. The session's sound is not a candidate -- that is the thing
// being cut.
func (ed *cutEditor) cardVoice(s *cutSeg) string {
	if s == nil || !ed.running() || insKind(s.Ins) != "video" {
		return ""
	}
