...
func tierIsNew(names []string, it tierItem) bool {
	for _, n := range names {
		...
		case strings.EqualFold(n, it.written()),
			...
			it.logo != "" && strings.EqualFold(n, logoName(it.logo)):
			return true
		}
	}
	return false
...
// logoName is what a logo is called when it has to stand in as text: the file's
// own name, without the folders and without the extension.
func logoName(p string) string {
	b := filepath.Base(p)
