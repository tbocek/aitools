...
// than a border -- right button to take hold, left drag to slide it, snapping to
...
// The right button already meant "pick up the border under the cursor". Away
// from any border it now means "pick up the whole clip", and the two readings
// must not overlap: the border is the smaller target and it lives inside the
// larger one.
func TestTheRightButtonPicksUpAWholeClip(t *testing.T) {
	ed := moveEd(t)
...
func TestACardIsHeldBeforeTheClipUnderIt(t *testing.T) {
	...
	// and the whole right press agrees: a border is asked about before a clip,
	...
	ed.pickAt(ed.xOf(40) - edgeGrab/2)
...
func TestASplicedCardIsMarkedLikeAHoleInTheFootage(t *testing.T) {
	...
	x := int(ed.xOf(20))
	band, ground := 0, 0
	for y := rulerH + 2; y < rulerH+ed.thumbHt; y++ {
	...
	solid := 0
	for y := rulerH + 1; y < rulerH+ed.thumbHt+4; y++ {
		r, g, b := at(x, y)
		if r > 90 && g > 70 && b < 90 {
			solid++
		}
...
func TestTheHeldClipIsDrawn(t *testing.T) {
	...
	white := func() int {
		at := renderTrack(t, ed, w, h)
		n := 0
		for y := rulerH; y < rulerH+ed.thumbHt+6; y++ {
...
// The right button picks something up, and picking something up does not move
...
func TestPickingSomethingUpNeverMovesTheRedLine(t *testing.T) {
	...
	ed.pickAt(ed.xOf(35)) // the middle of the second clip
	if s := ed.heldSeg(); s == nil || s.S != 30 {
		t.Fatalf("the held clip is %v, want the one starting at 30", s)
	}
	if ed.playhead != 5 {
	...
	ed.dropEdge()
	ed.pickAt(ed.xOf(30) + edgeGrab/2)
	if !ed.edgeOn || ed.edgeEnd || ed.edgeSeg != 1 {
...
func TestTheClipToolAndTheEditButtonAreWired(t *testing.T) {
	...
	for _, want := range []string{
		"ed.pickAt(x + ed.viewX)",                              // the right press, and all of what it means
		"case ed.grabSeg(px):",                                 // right button, away from a border
