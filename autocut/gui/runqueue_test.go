...
// Step 1 runs both halves at once, and the bar has to hold both without either
...
func TestTheStepsQueueTheWorkTheyDo(t *testing.T) {
	for _, c := range []struct {
		...
		{"pipeline.go", `a.qPush(trackSTT, len(inputs), "recording")`,
			"step 1 queues the recordings it has to listen to"},
		{"pipeline.go", `a.qPush(trackFrames, len(videos), "video")`,
			"step 1 queues the videos it has to open"},
		{"step2.go", `a.qJob(trackDescribe, "describe", 1, 2)`,
			"describe is not named as the first of the step's two jobs"},
		{"step2.go", `a.qJob(trackFix, "transcript", 2, 2)`,
			"transcript is not named as the second of the step's two jobs"},
		{"step2_describe.go", `a.qPush(trackDescribe, total, "chunk")`,
			"the chunks to describe are not queued"},
		{"step2_transcript.go", `a.qPush(trackFix, total, "block")`,
			"the blocks to fix are not queued"},
		{"step4.go", `a.qPush(trackSTT, len(speak), "line")`,
			"the lines to speak are not queued"},
		{"step5.go", `a.qPush(trackSTT, len(clips), "clip")`,
	...
	// already on disk
	b, err := os.ReadFile("step2_describe.go")
	if err != nil {
		t.Fatal(err)
	}
